 ...
 */
internal class AudiobookExtensionInstallReceiver(private val listener: Listener) :
    BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        when (intent.action) {
            Intent.ACTION_PACKAGE_ADDED, ACTION_EXTENSION_ADDED -> {
                ...
                launchNow {
                    when (val result = getExtensionFromIntent(context, intent)) {
                        is AudiobookLoadResult.Success -> listener.onExtensionInstalled(
                            result.extension,
                        )

                        is AudiobookLoadResult.Untrusted -> listener.onExtensionUntrusted(
                            result.extension,
                        )
                        else -> {}
                    }
            ...
            Intent.ACTION_PACKAGE_REPLACED, ACTION_EXTENSION_REPLACED -> {
                launchNow {
                    when (val result = getExtensionFromIntent(context, intent)) {
                        is AudiobookLoadResult.Success -> listener.onExtensionUpdated(result.extension)
     ...
     */
    private suspend fun getExtensionFromIntent(context: Context, intent: Intent?): AudiobookLoadResult {
        val pkgName = getPackageNameFromIntent(intent)
        if (pkgName == null) {
     ...
     */
    interface Listener {
        fun onExtensionInstalled(extension: AudiobookExtension.Installed)
        fun onExtensionUpdated(extension: AudiobookExtension.Installed)
        fun onExtensionUntrusted(extension: AudiobookExtension.Untrusted)
