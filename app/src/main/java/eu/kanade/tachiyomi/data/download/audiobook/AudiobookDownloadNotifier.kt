 ...
 */
internal class AudiobookDownloadNotifier(private val context: Context) {
     ...
     */
    fun onProgressChange(download: AudiobookDownload) {
        with(progressNotificationBuilder) {
            if (!isDownloading) {
                ...
                setContentIntent(
                    NotificationHandler.openAudiobookDownloadManagerPendingActivity(context),
                )
                isDownloading = true
                // Pause action
                addAction(
                    ...
                    NotificationReceiver.pauseAudiobookDownloadsPendingBroadcast(context),
                )
            }
            ...
            if (preferences.hideNotificationContent().get()) {
                ...
                val title = download.audiobook.title.chop(15)
    ...
    fun onPaused() {
        with(progressNotificationBuilder) {
            ...
            clearActions()
            // Open download manager when clicked
            setContentIntent(NotificationHandler.openAudiobookDownloadManagerPendingActivity(context))
            // Resume action
            addAction(
                R.drawable.ic_play_arrow_24dp,
                context.stringResource(MR.strings.action_resume),
                NotificationReceiver.resumeAudiobookDownloadsPendingBroadcast(context),
            )
            // Clear action
            addAction(
                R.drawable.ic_close_24dp,
                context.stringResource(MR.strings.action_cancel_all),
                NotificationReceiver.clearAudiobookDownloadsPendingBroadcast(context),
            )
     ...
     */
    fun onError(error: String? = null, chapter: String? = null, audiobookTitle: String? = null) {
        // Create notification
        with(errorNotificationBuilder) {
            setContentTitle(
                audiobookTitle?.plus(": $chapter") ?: context.stringResource(
