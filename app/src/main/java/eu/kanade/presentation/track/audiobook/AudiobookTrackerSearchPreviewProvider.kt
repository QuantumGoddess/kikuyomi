...
internal class AudiobookTrackerSearchPreviewProvider : PreviewParameterProvider<@Composable () -> Unit> {
    ...
    private fun someTrackSearches(): Sequence<AudiobookTrackSearch> = sequence {
        while (true) {
            yield(randTrackSearch())
        }
    }

    private fun randTrackSearch() = AudiobookTrackSearch().let {
        it.id = Random.nextLong()
        it.audiobook_id = Random.nextLong()
        ...
        it.started_listening_date = 0L
        it.finished_listening_date = 0L
