...
import eu.kanade.presentation.track.audiobook.AudiobookTrackInfoDialogHome
import eu.kanade.presentation.track.audiobook.AudiobookTrackerSearch
import eu.kanade.presentation.util.Screen
import eu.kanade.tachiyomi.data.track.AudiobookTracker
import eu.kanade.tachiyomi.data.track.DeletableAudiobookTracker
...
import tachiyomi.domain.track.audiobook.model.AudiobookTrack as DbAudiobookTrack

data class AudiobookTrackInfoDialogHomeScreen(
    private val audiobookId: Long,
    private val audiobookTitle: String,
    ...
    override fun Content() {
        ...
        val screenModel = rememberScreenModel { Model(audiobookId, sourceId) }
        ...
        AudiobookTrackInfoDialogHome(
            ...
            onNewSearch = {
                if (it.tracker is EnhancedAudiobookTracker) {
                    screenModel.registerEnhancedTracking(it)
                } else {
                    navigator.push(
                        TrackServiceSearchScreen(
                            audiobookId = audiobookId,
                            initialQuery = it.track?.title ?: audiobookTitle,
     ...
     */
    private fun openTrackerInBrowser(context: Context, trackItem: AudiobookTrackItem) {
    ...
    private class Model(
        ...
        init {
            ...
            screenModelScope.launch {
                getTracks.subscribe(audiobookId)
        ...
        fun registerEnhancedTracking(item: AudiobookTrackItem) {
            item.tracker as EnhancedAudiobookTracker
            screenModelScope.launchNonCancellable {
                val audiobook = Injekt.get<GetAudiobook>().await(audiobookId) ?: return@launchNonCancellable
                try {
                    val matchResult = item.tracker.match(audiobook) ?: throw Exception()
                    item.tracker.audiobookService.register(matchResult, audiobookId)
                } catch (e: Exception) {
        ...
        private suspend fun refreshTrackers() {
            val refreshTracks = Injekt.get<RefreshAudiobookTracks>()
            val context = Injekt.get<Application>()

            refreshTracks.await(audiobookId)
                .filter { it.first != null }
                .forEach { (track, e) ->
                    logcat(LogPriority.ERROR, e) {
                        "Failed to refresh track data mangaId=$audiobookId for service ${track!!.id}"
                    }
                    withUIContext {
                        context.toast(
        ...
        private fun List<AudiobookTrack>.mapToTrackItem(): List<AudiobookTrackItem> {
            val loggedInTrackers = Injekt.get<TrackerManager>().trackers.filter {
                it.isLoggedIn && it is AudiobookTracker
            }
            val source = Injekt.get<AudiobookSourceManager>().getOrStub(sourceId)
            return loggedInTrackers
                // Map to TrackItem
                .map { service -> AudiobookTrackItem(find { it.syncId.toLong() == service.id }, service) }
...
private data class TrackStatusSelectorScreen(
    private val track: DbAudiobookTrack,
    ...
    private class Model(
        ...
        fun getSelections(): Map<Int, StringResource?> {
            return tracker.audiobookService.getStatusListAudiobook().associateWith { tracker.getStatus(it) }
        }
        ...
        fun setStatus() {
            screenModelScope.launchNonCancellable {
                tracker.audiobookService.setRemoteAudiobookStatus(track.toDbTrack(), state.value.selection)
            }
        }
...
private data class TrackChapterSelectorScreen(
    ...
    private class Model(
        ...
        fun setChapter() {
            screenModelScope.launchNonCancellable {
                tracker.audiobookService.setRemoteLastChapterRead(
                    track.toDbTrack(),
...
private data class TrackScoreSelectorScreen(
    ...
    private class Model(
        private val track: DbAudiobookTrack,
        private val tracker: Tracker,
    ) : StateScreenModel<Model.State>(State(tracker.audiobookService.displayScore(track.toDbTrack()))) {

        fun getSelections(): ImmutableList<String> {
            return tracker.audiobookService.getScoreList()
        }
        ...
        fun setScore() {
            screenModelScope.launchNonCancellable {
                tracker.audiobookService.setRemoteScore(track.toDbTrack(), state.value.selection)
            }
        }
...
private data class TrackDateSelectorScreen(
    ...
    private class Model(
        ...
        fun setDate(millis: Long) {
            ...
            screenModelScope.launchNonCancellable {
                if (start) {
                    tracker.audiobookService.setRemoteStartDate(track.toDbTrack(), localMillis)
                } else {
                    tracker.audiobookService.setRemoteFinishDate(track.toDbTrack(), localMillis)
                }
            }
        }
...
private data class TrackDateRemoverScreen(
                ...
                Row(
                    ...
                    FilledTonalButton(
                        onClick = {
                            ...
                            navigator.popUntil { it is AudiobookTrackInfoDialogHomeScreen }
                        },
    ...
    private class Model(
        ...
        fun removeDate() {
            screenModelScope.launchNonCancellable {
                if (start) {
                    tracker.audiobookService.setRemoteStartDate(track.toDbTrack(), 0)
                } else {
                    tracker.audiobookService.setRemoteFinishDate(track.toDbTrack(), 0)
                }
            }
        }
    }
...
data class TrackServiceSearchScreen(
    ...
    private class Model(
        ...
        fun trackingSearch(query: String) {
            screenModelScope.launch {
                ...
                val result = withIOContext {
                    try {
                        val results = tracker.audiobookService.searchAudiobook(query)
        ...
        fun registerTracking(item: AudiobookTrackSearch) {
            screenModelScope.launchNonCancellable { tracker.audiobookService.register(item, audiobookId) }
        }

        fun updateSelection(selected: AudiobookTrackSearch) {
            mutableState.update { it.copy(selected = selected) }
        }

        @Immutable
        data class State(
            val queryResult: Result<List<AudiobookTrackSearch>>? = null,
            val selected: AudiobookTrackSearch? = null,
        )
    }
}

private data class TrackerAudiobookRemoveScreen(
    private val audiobookId: Long,
    private val track: AudiobookTrack,
    private val serviceId: Long,
) : Screen() {
                ...
                Row(
                    ...
                    FilledTonalButton(
                        onClick = {
                            ...
                            if (removeRemoteTrack) screenModel.deleteAudiobookFromService()
                            navigator.pop()
                        },
                        colors = ButtonDefaults.filledTonalButtonColors(
                            containerColor = MaterialTheme.colorScheme.errorContainer,
    ...
    private class Model(
        ...
        private val track: AudiobookTrack,
        private val tracker: Tracker,
        private val deleteTrack: DeleteAudiobookTrack = Injekt.get(),
    ) : ScreenModel {

        fun getName() = tracker.name

        fun isDeletable() = tracker is DeletableAudiobookTracker

        fun deleteAudiobookFromService() {
            screenModelScope.launchNonCancellable {
                (tracker as DeletableAudiobookTracker).delete(track.toDbTrack())
            }
        }

        fun unregisterTracking(serviceId: Long) {
            screenModelScope.launchNonCancellable { deleteTrack.await(audiobookId, serviceId) }
        }
    }
}
