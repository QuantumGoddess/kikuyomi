...
import eu.kanade.tachiyomi.audiobooksource.AudiobookSourceFactory
...
internal object AudiobookExtensionLoader {
    ...
    private const val EXTENSION_FEATURE = "tachiyomi.audiobookextension"
    private const val METADATA_SOURCE_CLASS = "tachiyomi.audiobookextension.class"
    private const val METADATA_SOURCE_FACTORY = "tachiyomi.audiobookextension.factory"
    private const val METADATA_NSFW = "tachiyomi.audiobookextension.nsfw"
    private const val METADATA_HAS_README = "tachiyomi.audiobookextension.hasReadme"
    private const val METADATA_HAS_CHANGELOG = "tachiyomi.audiobookextension.hasChangelog"
    ...
    fun installPrivateExtensionFile(context: Context, file: File): Boolean {
        ...
        val currentExtension = getAudiobookExtensionPackageInfoFromPkgName(
            context,
        ...
        return try {
            ...
            if (currentExtension != null) {
                AudiobookExtensionInstallReceiver.notifyReplaced(context, extension.packageName)
            } else {
                AudiobookExtensionInstallReceiver.notifyAdded(context, extension.packageName)
            }
            true
        } catch (e: Exception) {
     ...
     */
    fun loadExtensions(context: Context): List<AudiobookLoadResult> {
        ...
        val sharedExtPkgs = installedPkgs
            ...
            .map { AudiobookExtensionInfo(packageInfo = it, isShared = true) }
        ...
        val privateExtPkgs = getPrivateExtensionDir(context)
            ...
            ?.filter { isPackageAnExtension(it) }
            ?.map { AudiobookExtensionInfo(packageInfo = it, isShared = false) }
            ?: emptySequence()
     ...
     */
    fun loadExtensionFromPkgName(context: Context, pkgName: String): AudiobookLoadResult {
        val extensionPackage = getAudiobookExtensionInfoFromPkgName(context, pkgName)
        if (extensionPackage == null) {
            logcat(LogPriority.ERROR) { "Extension package is not found ($pkgName)" }
            return AudiobookLoadResult.Error
        }
        return loadExtension(context, extensionPackage)
    }

    fun getAudiobookExtensionPackageInfoFromPkgName(context: Context, pkgName: String): PackageInfo? {
        return getAudiobookExtensionInfoFromPkgName(context, pkgName)?.packageInfo
    }

    private fun getAudiobookExtensionInfoFromPkgName(context: Context, pkgName: String): AudiobookExtensionInfo? {
     ...
     */
    private fun loadExtension(context: Context, extensionInfo: AudiobookExtensionInfo): AudiobookLoadResult {
        val pkgManager = context.packageManager
        ...
        if (signatures.isNullOrEmpty()) {
            ...
            val extension = AudiobookExtension.Untrusted(
                extName,
                pkgName,
                versionName,
                versionCode,
            ...
            return AudiobookLoadResult.Untrusted(extension)
        }
            ...
            .flatMap {
                try {
                    when (val obj = Class.forName(it, false, classLoader).getDeclaredConstructor().newInstance()) {
                        is AudiobookSource -> listOf(obj)
                        is AudiobookSourceFactory -> obj.createSources()
        ...
        val langs = sources.filterIsInstance<AudiobookCatalogueSource>()
            .map { it.lang }
            .toSet()
        ...
        val extension = AudiobookExtension.Installed(
            name = extName,
            pkgName = pkgName,
        ...
        return AudiobookLoadResult.Success(extension)
    }
     ...
     */
    private fun selectExtensionPackage(shared: AudiobookExtensionInfo?, private: AudiobookExtensionInfo?): AudiobookExtensionInfo? {
        when {
    ...
    private data class AudiobookExtensionInfo(
