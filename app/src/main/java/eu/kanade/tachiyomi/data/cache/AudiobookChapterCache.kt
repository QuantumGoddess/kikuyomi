 ...
 * For each image in an chapter a file is created
 ...
 */
class AudiobookChapterCache(private val context: Context) {

    private val json: Json by injectLazy()

    /** Cache class used for cache management. */
    private val diskCache = DiskLruCache.open(
        File(context.cacheDir, "audiobookchapter_disk_cache"),
     ...
     */
    fun putPageListToCache(chapter: Chapter, audio: Audio) {
        // Convert list of pages to json string.
        val cachedValue = json.encodeToString(audio)
        ...
        try {
            ...
            logcat(LogPriority.WARN, e) { "Failed to put audio list to cache" }
            // Ignore.
        } finally {
            editor?.abortUnlessCommitted()
        }
     ...
     */
    fun getAudioFile(imageUrl: String): File {
    ...
    private fun getKey(chapter: Chapter): String {
        return "${chapter.audiobookId}${chapter.url}"
    }
}
