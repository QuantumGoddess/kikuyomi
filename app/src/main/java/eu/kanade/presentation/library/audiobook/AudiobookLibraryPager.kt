...
@Composable
fun AudiobookLibraryPager(
    state: PagerState,
    contentPadding: PaddingValues,
    hasActiveFilters: Boolean,
    selectedAudiobook: List<LibraryAudiobook>,
    searchQuery: String?,
    onGlobalSearchClicked: () -> Unit,
    getDisplayMode: (Int) -> PreferenceMutableState<LibraryDisplayMode>,
    getColumnsForOrientation: (Boolean) -> PreferenceMutableState<Int>,
    getLibraryForPage: (Int) -> List<AudiobookLibraryItem>,
    onClickAudiobook: (LibraryAudiobook) -> Unit,
    onLongClickAudiobook: (LibraryAudiobook) -> Unit,
    ...
    HorizontalPager(
        ...
        when (displayMode) {
            LibraryDisplayMode.List -> {
                AudiobookLibraryList(
                    items = library,
                    contentPadding = contentPadding,
                    selection = selectedAudiobook,
                    onClick = onClickAudiobook,
                    onClickContinueListening = onClickContinueListening,
                    onLongClick = onLongClickAudiobook,
                    searchQuery = searchQuery,
                    onGlobalSearchClicked = onGlobalSearchClicked,
                )
            }
            LibraryDisplayMode.CompactGrid, LibraryDisplayMode.CoverOnlyGrid -> {
                AudiobookLibraryCompactGrid(
                    items = library,
            ...
            LibraryDisplayMode.ComfortableGrid -> {
                AudiobookLibraryComfortableGrid(
                    items = library,
                    columns = columns,
                    contentPadding = contentPadding,
                    selection = selectedAudiobook,
