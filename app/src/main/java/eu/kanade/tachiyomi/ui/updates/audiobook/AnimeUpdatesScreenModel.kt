...
import eu.kanade.presentation.updates.audiobook.AudiobookUpdatesUiModel
...
class AudiobookUpdatesScreenModel(
    ...
    private val getUpdates: GetAudiobookUpdates = Injekt.get(),
...
) : StateScreenModel<AudiobookUpdatesScreenModel.State>(State()) {
    ...
    init {
        ...
        screenModelScope.launchIO {
            merge(downloadManager.statusFlow(), downloadManager.progressFlow())
                ...
                .collect(this@AudiobookUpdatesScreenModel::updateDownloadState)
        }
    }

    private fun List<AudiobookUpdatesWithRelations>.toUpdateItems(): PersistentList<AudiobookUpdatesItem> {
        return this
            .map { update ->
    ...
    fun updateLibrary(): Boolean {
        val started = AudiobookLibraryUpdateJob.startNow(Injekt.get<Application>())
    ...
    fun downloadChapters(items: List<AudiobookUpdatesItem>, action: ChapterDownloadAction) {
        if (items.isEmpty()) return
        screenModelScope.launch {
            when (action) {
                ChapterDownloadAction.START -> {
                    downloadChapters(items)
                    if (items.any { it.downloadStateProvider() == AudiobookDownload.State.ERROR }) {
     ...
     */
    fun markUpdatesRead(updates: List<AudiobookUpdatesItem>, read: Boolean) {
        screenModelScope.launchIO {
            setReadStatus.await(
                read = read,
                chapters = updates
     ...
     */
    fun bookmarkUpdates(updates: List<AudiobookUpdatesItem>, bookmark: Boolean) {
        screenModelScope.launchIO {
            updates
     ...
     */
    private fun downloadChapters(updatesItem: List<AudiobookUpdatesItem>, alt: Boolean = false) {
        screenModelScope.launchNonCancellable {
            val groupedUpdates = updatesItem.groupBy { it.update.audiobookId }.values
            for (updates in groupedUpdates) {
                val audiobookId = updates.first().update.audiobookId
                val audiobook = getAudiobook.await(audiobookId) ?: continue
                // Don't download if source isn't available
                sourceManager.get(audiobook.source) ?: continue
                val chapters = updates.mapNotNull { getChapter.await(it.update.chapterId) }
                downloadManager.downloadChapters(audiobook, chapters, true, alt)
            }
        }
    }
     ...
     */
    fun deleteChapters(updatesItem: List<AudiobookUpdatesItem>) {
        screenModelScope.launchNonCancellable {
            updatesItem
                .groupBy { it.update.audiobookId }
                .entries
                .forEach { (audiobookId, updates) ->
                    val audiobook = getAudiobook.await(audiobookId) ?: return@forEach
                    val source = sourceManager.get(audiobook.source) ?: return@forEach
                    val chapters = updates.mapNotNull { getChapter.await(it.update.chapterId) }
                    downloadManager.deleteChapters(chapters, audiobook, source)
                }
        }
        toggleAllSelection(false)
    }

    fun showConfirmDeleteChapters(updatesItem: List<AudiobookUpdatesItem>) {
        setDialog(Dialog.DeleteConfirmation(updatesItem))
    }

    private fun showQualitiesDialog(update: AudiobookUpdatesWithRelations) {
        setDialog(
    ...
    fun resetNewUpdatesCount() {
        libraryPreferences.newAudiobookUpdatesCount().set(0)
    }

    @Immutable
    data class State(
        val isLoading: Boolean = true,
        val items: PersistentList<AudiobookUpdatesItem> = persistentListOf(),
        val dialog: Dialog? = null,
    ) {
        val selected = items.filter { it.selected }
        val selectionMode = selected.isNotEmpty()

        fun getUiModel(context: Context, relativeTime: Boolean): List<AudiobookUpdatesUiModel> {
            val dateFormat by mutableStateOf(
                UiPreferences.dateFormat(Injekt.get<UiPreferences>().dateFormat().get()),
            )

            return items
                .map { AudiobookUpdatesUiModel.Item(it) }
                .insertSeparators { before, after ->
                    ...
                    when {
                        beforeDate.time != afterDate.time && afterDate.time != 0L -> {
                            ...
                            AudiobookUpdatesUiModel.Header(text)
                        }
    ...
    sealed interface Dialog {
        data class DeleteConfirmation(val toDelete: List<AudiobookUpdatesItem>) : Dialog
...
@Immutable
data class AudiobookUpdatesItem(
    val update: AudiobookUpdatesWithRelations,
    val downloadStateProvider: () -> AudiobookDownload.State,
